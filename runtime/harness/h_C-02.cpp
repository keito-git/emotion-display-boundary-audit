...
int main() {
    // State A (pre-registered): 'anger' and 'surprise' absent from the upstream map
    run("A_missing", {{"neutral", 0.10f}, {"happy", 0.90f}, {"sad", 0.00f}});
    // State B (pre-registered): the same two labels present and measured at exactly 0.0
    run("B_measured_zero", {{"neutral", 0.10f}, {"happy", 0.90f}, {"sad", 0.00f},
                            {"surprise", 0.0f}, {"anger", 0.0f}});
    // Control: a genuinely different upstream state must NOT collapse onto A
    run("C_control_different", {{"neutral", 0.10f}, {"happy", 0.90f}, {"sad", 0.00f},
                                {"surprise", 0.30f}, {"anger", 0.0f}});
    // Extra: a tiny but non-zero measurement -- does int truncation collapse it too?
    run("D_tiny_0004", {{"neutral", 0.10f}, {"happy", 0.90f}, {"sad", 0.00f},
                        {"surprise", 0.004f}, {"anger", 0.0f}});
    probe_insertion();
    return 0;
}
